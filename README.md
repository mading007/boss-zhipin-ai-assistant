...
这是项目里花时间最多、也最值得记录的部分——前后推翻过三次判断。
...
| 讲哪段经历 | 默认永远讲同一个，一批 10 条话术正文完全一致 | 按 JD 关键词在项目间匹配，无命中则走默认项 |

结论：**对于「必须按条件走分支」的需求，不要让模型做判断，把判断放在代码里。**
现在这四种开头、项目选择、链接拼接、长度截断全部由 Python 控制，行为可预测、可测试。

#### 再解决「话术为什么读起来水」

改完上面这些，话术确实稳定了，但**内容很空**。一批 40 条里有 20 条长这样：

> 我独立用 LangChain + Chroma + BGE Embedding 搭过一套 RAG **论文**问答系统，
> 从文档切分到答案溯源完整跑通。

两个毛病：

1. **「论文」两个字暴露了这是课程作业** —— 技术栈再对，第一印象已经打折
2. **通篇在讲「我怎么实现的」，没讲「做出来能干嘛」** —— HR 扫过去既抓不到可匹配的关键词，
   也不知道这东西有什么价值

改法是把话术的结构倒过来：**先亮技术名（关键词），再说清拿它做成了什么（价值）**。

> 我用 **LangChain** 把 PDF 等文档解析、切分、向量化后做语义检索，
> 回答带原文出处，**能直接接进业务系统**；也常用 **Python** 处理数据、对接数据库。

对应到实现上，每个项目多了两个字段：

| 字段 | 作用 |
|---|---|
| `tools` | 可被念出来的技术名，作为关键词前置 |
| `desc` | 讲业务价值（做出来能干嘛），不讲实现细节 |

另外从 JD 里**提取技术词注入提示词**（`extract_jd_tech()`，77 个词的词表按「具体→泛化」排序），
让模型知道这个岗位最在意什么，点名呼应 1-2 个。实测提取效果：
...
这部分踩过坑：早期为了「不让话术变成技能清单」，写了三条禁令，结果**约束过度**，
模型只能写通用的话，话术反而更水。
...
| **事实** | 提示词（硬规则） | 不许编造经历、不许写回复率数据、不放网址 |
| **风格与内容** | 放开给模型 | 挑哪个技术点、怎么说价值、语气用词 |

> 一句话：**把判断放代码里，但别把语言也锁死。**
...
【要求】        若干条规则，其中三条由 Python 预先填好变量
招呼语：        让模型直接续写正文
```

三个注入变量——把本该由模型判断的事提前在代码里决定：

```python
opening   = _pick_opening(row)          # 轮换选定开头，prompt 里只写「原样照抄这句」
project   = _pick_project(row, profile) # 按 JD 关键词选定讲哪段经历
jd_tech   = extract_jd_tech(jd)         # 从 JD 提取技术词，点名让模型呼应
```
...
> **两个实测出来的细节**：
>
> - 第 2 条一开始写的是「把技术名念出来」，结果模型把三个技术名**全念了**，
>   又变回技能清单。收紧成「挑 1 个」之后才是想要的效果。
> - 画像要截断到 320 字：把 4 个项目完整灌进 prompt 时，模型会贪心地把它们
>   **全部**写进话术，导致输出超长（实测 96~115 字，而要求是 100 字以内）。
>   只给 320 字素材后，模型只能在有限材料里取舍，输出自然收敛。
>   ——**字数问题不能靠调字数限制解决，要改输入。**

#### 长度兜底

模型无视字数限制（实测仍会写到 115 字），所以代码兜底到 110 字。
但不能硬切——会出现「…能直接。」这种半句话。做法是先给收尾句
「期待有机会进一步沟通。」预留预算，再截正文；正文按句末回退，不切断句子。
